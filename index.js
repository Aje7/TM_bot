...
ban = []
cr = '*TMBOT*'
vz = '*Terima Kasih Sudah Daftar Di TMBOT*'
		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: 'Maaf Kak Fitur Ini Untuk User Mods TMbot',
					...
					userB: `──「 DAFTAR 」── \n 𝖧𝖺𝗅𝗅𝗈 𝖬𝖻𝖺𝗄 𝗆𝖺𝗌 𝖭𝗒𝖺 𝖸𝖺𝗇𝗀 𝖦𝖺𝗇𝗍𝖾𝗇𝗀 𝖽𝖺𝗇 𝖢𝖺𝗇𝗍𝗂𝗄 \n 𝖪𝖺𝗆𝗎 𝖡𝖾𝗅𝗎𝗆 𝖳𝖾𝗋𝖽𝖺𝖿𝗍𝖺𝗋 𝖣𝗂 TMbot 𝖠𝗒𝗈 𝖡𝗎𝗋𝗎𝖺𝗇 𝖽𝖺𝖿𝗍𝖺𝗋 𝖪𝖺𝗄 𝖡𝗂𝖺𝗋 𝖻𝗂𝗌𝖺 𝗆𝖾𝗇𝗀𝖺𝗄𝗌𝖾𝗌 𝖥𝗂𝗍𝗎𝗋 \n 𝖢𝖺𝗋𝖺 𝖽𝖺𝖿𝗍𝖺𝗇𝗒𝖺 𝖦𝗂𝗇𝗂 𝖪𝖺𝗄 \n\n/𝖣𝖺𝖿𝗍𝖺𝗋 Zopirul |17 \n\n──「 TMBOT 」──`,
			...
			const ownerNumber = ["628pasangnomoryou@s.whatsapp.net","6281388981898@s.whatsapp.net"]
			const mod = [ownerNumber,"628pasangnomoryou@s.whatsapp.net","6281388981898@s.whatsapp.net"]
			const adminbotnumber = ["628pasangnomoryou@s.whatsapp.net","628pasangnomoryou@s.whatsapp.net"]
			const frendsowner = ["628pasangnomoryou@s.whatsapp.net","628pasangnomoryou@s.whatsapp.net"]
			const premium = ["628pasangnomoryou@s.whatsapp.net","628pasangnomoryou@s.whatsapp.net","628pasangnomoryou@s.whatsapp.net","628pasangnomoryou@s.whatsapp.net","628pasangnomoryou@s.whatsapp.net","628pasangnomoryou@s.whatsapp.net"]
			...
			switch(command) {
				...
				if (!isUser) return reply(mess.only.userB)
			       ...
			       case 'valxlove':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMIN TMBOT ✨* 」\n*│+wa.me/628pasangnomoryou*╰──────*「 *TMBOT* 」*────*\n\n*_JIKA INGIN MENJADI ADMIN  TMBOT_*\n*_Ketik /iklan_*' })
					break
					case 'premiumcek':
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
					sa = await getBuffer(`https://i.ibb.co/PcQ6tsB/79ac87b9358c.jpg`)
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*╭────*「 *PREMIUM USER👑* 」\n*│+wa.me/628pasangnomoryou*╰──────*「 *TMBOT* 」*────*\n\n*_JIKA INGIN MENJADI PREMIUM USER  TMBOT_*\n*_Ketik /daftarvip*' })
					break
					case 'cekmod': 
 ...
 if (isBanned) return reply(mess.only.benned)    
			...
			if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					...
					setTimeout( () => {
					 client.sendMessage(from, 'siap siap ya TMBOT Kick nih', text, { quoted: mek }) // ur cods
					}, 0) // 1000 = 10s,
					}
					break
				case 'aguse':
 ...
 if (!isOwner) return reply(mess.only.ownerB)
			...
			if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					...
					mentions('╔══✪〘 Mention All 〙✪══'+teks+'╚═〘  TMBOT 〙', members_id, true)
					break
				case 'clearall':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *TMBOT Broadcast* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
	...
	.then(() => {client.sendMessage(from, options, text)})
				...
				//frendowner
				case `addadmin`:
                   if (!isOwner) return reply('Perintah ini hanya bisa di gunakan oleh Owner TMBOT!', id)
                   for (let i = 0; i < mentionedJidList.length; i++) {
                   adminNumber.push(mentionedJidList[i])
                   fs.writeFileSync('./lib/database/admin.json', JSON.stringify(adminNumber))
                   reply('Success Menambahkan Admin TMBOT!')
				   }
                   break
				case 'wait':
				if (isBanned) return reply(mess.only.benned)    
